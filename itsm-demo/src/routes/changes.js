...
router.get('/changes', (req, res) => {
  ...
  const allowedSort = new Set(['opened_at','priority','state','planned_start','planned_end','last_updated']);
  const sortCol = allowedSort.has(sort) ? sort : 'opened_at';
  const sortOrder = order?.toLowerCase() === 'asc' ? 'ASC' : 'DESC';

  const whereSql = where.length ? `WHERE ${where.join(' AND ')}` : '';
  const total = db.prepare(`SELECT COUNT(1) as c FROM changes ${whereSql}`).get(params).c;
  const rows = db.prepare(`SELECT * FROM changes ${whereSql} ORDER BY ${sortCol} ${sortOrder} LIMIT @limit OFFSET @offset`).all({ ...params, limit: Number(limit), offset: Number(offset) });
